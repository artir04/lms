      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-semibold text-white truncate">{assignment.title}</h4>
            {!assignment.is_published && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-700/50 text-slate-400 uppercase tracking-wider shrink-0">
                Draft
              </span>
            )}
            {submissionStatus && (
              <span className={cn('text-[10px] px-1.5 py-0.5 rounded-full uppercase tracking-wider shrink-0', STATUS_COLORS[submissionStatus] ?? 'text-slate-400 bg-slate-700/50')}>
            ...
            <>
              <Link to={submissionsUrl} className="btn-secondary text-xs h-8">
                ...
                {assignment.submission_count != null && assignment.submission_count > 0 && (
                  <span className="ml-1 text-slate-400">({assignment.submission_count})</span>
                )}
              </Link>
