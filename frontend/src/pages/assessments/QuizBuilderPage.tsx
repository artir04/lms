        ...
        {quiz.questions?.map((q, idx) => (
          <div key={q.id} className="card p-5">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                <p className="text-xs text-gray-400 mb-1">Q{idx + 1} · {QUESTION_TYPES.find((t) => t.value === q.question_type)?.label} · {q.points} pts</p>
                <p className="font-medium text-gray-900">{q.text}</p>
                {q.options.length > 0 && (
                  <ul className="mt-3 space-y-1.5">
                    {q.options.map((opt) => (
                      <li key={opt.id} className={`flex items-center gap-2 text-sm rounded px-3 py-1.5 ${opt.is_correct ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-600'}`}>
                ...
                {q.explanation && <p className="mt-2 text-xs text-gray-400 italic">Explanation: {q.explanation}</p>}
              </div>
            </div>
          </div>
        ))}
      ...
      <Modal isOpen={showSettings} onClose={() => setShowSettings(false)} title="Quiz Settings">
        <form onSubmit={settingsForm.handleSubmit((d) => updateQuiz({ ...d, time_limit_min: d.time_limit_min ? Number(d.time_limit_min) : null }, { onSuccess: () => setShowSettings(false) }))} className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="qz_published" className="text-sm text-gray-700">Published (visible to students)</label>
          </div>
              ...
              <div className="space-y-2">
                {options.map((opt, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <button type="button" onClick={() => toggleCorrect(i)} className={`w-5 h-5 rounded-full border-2 flex-shrink-0 ${opt.is_correct ? 'bg-green-500 border-green-500' : 'border-gray-300'}`} />
                    {questionType === 'true_false' ? (
                      <span className="flex-1 input bg-gray-50">{opt.text}</span>
                    ) : (
